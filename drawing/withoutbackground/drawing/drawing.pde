...
void setup() {
  ...
  //piano
  piano = new Piano[8];
  piano[0]= new Piano("C3");
  piano[1]= new Piano("D3");
  piano[2]= new Piano("E3");
  piano[3]= new Piano("F3");
  piano[4]= new Piano("G3");
  piano[5]= new Piano("A3");
  piano[6]= new Piano("B3");
  ...
  for (int j=0; j<lines.size (); j++) {
    ...
    for (int i =0; i < dots.size ()-1; i++) {
      ...
      if (i==playHeads.get(j)) {
        ...
        if (dots.get(i).x<width/8 && dots.get(i).x>0) {
          piano[0].play();
        }
        if (dots.get(i).x<2*width/8 && dots.get(i).x>width/8) {
          piano[1].play();
        }
        if (dots.get(i).x<3*width/8 && dots.get(i).x>2*width/8) {
          piano[2].play();
        }
        if (dots.get(i).x<4*width/8 && dots.get(i).x>3*width/8) {
          piano[3].play();
        }
        if (dots.get(i).x<5*width/8 && dots.get(i).x>4*width/8) {
          piano[4].play();
        }
        if (dots.get(i).x<6*width/8 && dots.get(i).x>5*width/8) {
          piano[5].play();
        }
        if (dots.get(i).x<7*width/8 && dots.get(i).x>6*width/8) {
          piano[6].play();
        }
        if (dots.get(i).x<width && dots.get(i).x>7*width/8) {
          piano[7].play();
        }
        //popMatrix();
      }
    ...
    for (int k=0; k<dots.size ()-1; k++) {
      for (int x=0; x<k; x++) {
        ...
        if (k == playHeads.get(l)) {
          if (x == playHeads.get(l)) {
            if (x != k) {      
              float dist_playhead = 0;
              dist_playhead= dist(dots.get(x).x, dots.get(x).y, dots.get(k).x, dots.get(k).y);

              if (dist_playhead < pDistance) {
                float strokeR=map(dots.get(x).x, 0, width, 0, 255);
                float strokeG=map(dots.get(x).y, 0, width, 0, 255);
                stroke(strokeR, strokeG, strokeB);
                strokeWeight(0.4);
                line(dots.get(x).x, dots.get(x).y, dots.get(k).x, dots.get(k).y);
              }
            }
          }
        }
